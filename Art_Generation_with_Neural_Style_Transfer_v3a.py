...
def compute_content_cost(a_C, a_G):
    ...
    J_content = (tf.reduce_sum(np.square(a_C_unrolled-a_G_unrolled), axis=None))/(4*n_H*n_W*n_C)
    ### END CODE HERE ###
...
def gram_matrix(A):
    ...
    GA =tf.matmul(
        A, A, transpose_a=False, transpose_b=True)
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GS,GG)))*(1/(4*np.square(n_C)*np.square(n_H*n_W)))
